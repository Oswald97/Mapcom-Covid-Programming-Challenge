...
nodesSccNum = []
scc = [[]]
dfsNumberCounter, numSCC = 0,0
AdjList = [[(0,0)]]
revGraph = [[(0,0)]]

def tarjanSCC(u):
    dfs_low[u] = dfs_num[u] = dfsNumberCounter++ # dfs_low[u] <= dfs_num[u]
    S.append(u) # stores u in a vector based on order of visitation
    visited[u] = 1
    for j in range(len(AdjList[u])):
        ...
        if dfs_num[v[0]] == 0:
            tarjanSCC(v[0])
        if visited[v[0]]:
            dfs_low[u] = min(dfs_low[u], dfs_low[v[0]])
    ...
    if dfs_low[u] == dfs_num[u]:
        ...
        while True:
            ...
            visited[v] = 0
            scc[numSCC].append(v)
            nodesSccNum[v] = numSCC
            if u == v:
                break


void dfs(vis,i,graph) {
    vis[i] = 1
    for e in graph[i]:
        if !vis[e[0]]:
            dfs(vis, e[0], graph)
